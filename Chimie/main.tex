...
\usepackage{amsfonts}
\usepackage{graphicx}
\usepackage{makecell} % Pour permettre de créer des cellules multilignes
\usepackage{diagbox}
\usepackage[colorlinks=true, linkcolor=blue!35!black, citecolor=blue, urlcolor=blue]{hyperref}
\usepackage{siunitx}
\usepackage{bm}
\usepackage{graphicx}
\usepackage{pstricks}
...
\usepackage{collect}
\usepackage{tikz}
\definecolor{darkgreen}{rgb}{0,0.5,0} % Define dark green
...
\usepackage{nicefrac}
\newcommand*{\getTitle}{Architecture de la matière}

\usepackage{lastpage}
\usepackage{fancyhdr} % Load the package
\usepackage{titlesec} % Load the titlesec package for conditional styling

% Define the "plain" pagestyle
...
% Define a custom page style for "non-chapter" pages
...
% Set the default pagestyle to "non-chapter-style"
\pagestyle{non-chapter-style}

\usepackage{tocloft}
% Adjust spacing in the table of contents
\setlength{\cftsecnumwidth}{3em} % Increase space between number and title
\DeclareSIUnit{\angstrom}{\text{\r{A}}} % declare angstrom as a unit

% Define a custom font size smaller than \tiny
\newcommand\Tiny{\fontsize{3.4}{5}\selectfont}

% Customize section numbering and enforce page behavior
\renewcommand{\thesection}{\Roman{section}} % Customize section format
\renewcommand{\thesubsection}{\arabic{subsection}} % Customize subsection format
\renewcommand{\thesubsubsection}{\thesubsection.\alph{subsubsection}} % Customize subsubsection format
...
\renewcommand{\section}{%
  ...
  \@startsection{section}{1}{\z@}% Arguments for section formatting
    {3.5ex \@plus 1ex \@minus .2ex}% Space before the section title
    {2.3ex \@plus .2ex}% Space after the section title
    {\normalfont\Large\bfseries}% Make the section title bold and large
}
\makeatother

% define command to display cases quantiques
...
% Define command to duplicate tables with the same label
...
C'est la deuxième version mais il manque toujours des exercices d'applications. (En cristallo par exmemple)
...
\begin{figure}[h]
    ...
    \begin{subfigure}{0.48\textwidth}
        \centering
        \begin{tikzpicture}[scale=3, x={(1.1,0)}, y={(0,1.2)}, z={(0, 0, 1.2)}, line join=round, line cap=round]
