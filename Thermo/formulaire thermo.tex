\documentclass[a4paper,12pt]{article}

% Encodage et langue
\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage[french]{babel}

% Mathématiques
\usepackage{amsmath, amssymb, amsfonts}
\usepackage{siunitx} % Pour les unités
\sisetup{locale=FR} % Configuration pour le français
\usepackage{bm} % bold maths

% Graphiques et figures
\usepackage{graphicx}
\usepackage{enumitem} % pour les puces de listes
\usepackage{siunitx}
\usepackage{tikz}

% Mise en page
\usepackage{geometry}
\geometry{a4paper, margin=1.5cm}
\usepackage{multicol}

% Couleurs et hyperliens
\usepackage{xcolor}
\usepackage{hyperref}
\hypersetup{
    colorlinks=true,
    linkcolor=blue,
    citecolor=blue,
    urlcolor=blue
}

% Commandes personnalisées
\newcommand{\diff}{\mathrm{d}} % Pour les différentielles droites
\newcommand{\kB}{k_\mathrm{B}} % Constante de Boltzmann
\newcommand{\pV}{pV} % Produit pression-volume
\newcommand{\re}{\textrm}
\newcommand{\Cp}{\overline{C}_p} % definition de Cp molaire
\newcommand{\Cv}{\overline{C}_v} % definition de Cv molaire


\title{Formulaire de Thermodynamique}
\author{Raphaël Jamann}
\date{}

\begin{document}
\maketitle


\section{Definitions des grandeurs en Thermodynamique}

    \begin{multicols}{2}

    \subsection*{Système}

    \textit{Un système est une portion de l'Univers que l'on isole du reste,
    ce dernier constituant alors le milieu extérieur.}\\
    Il y a trois types de système :
    \begin{itemize}[label=\textbullet]
        \item \textbf{ouvert} s'il échange énergie et matière.
        \item \textbf{fermé} s'il n'échange que de l'énergie.
        \item \textbf{isolé} s'il n'échange ni énergie, ni matière.
    \end{itemize}

    \subsection*{Variables d'état}
    Un système est défini par des \textbf{variables d'état} qui peuvent être :
    \begin{itemize}[label=\textbullet]
        \item \textbf{Intensives} : ne dépendent pas de la quantité de matière (ex: pression, température, etc\dots)
        \item \textbf{Extensives} : dépendent de la quantité de matière (ex: volume, énergie interne, entropie, etc\dots)
    \end{itemize}


    \subsection*{Énergie interne $\bm{U}$}
        En joule, cette énergie est une fonction d'état (ne dépend pas du chemin suivie lors d'une transformation).
        $$ \boxed{ \diff U = C_v \, \diff T + (l - P) \, \diff V } $$

    \subsection*{Chaleur $\bm{Q}$}
        Energie calorifique échangé au cours d'une transformation.
            $$ \boxed{ Q_{\text{rév}} = C_v \, \diff T + l \, \diff V } $$
            $$ \boxed{ Q_{\text{rév}} = C_p \, \diff T + h \, \diff P } $$

    \subsection*{Travail $\bm{W}$}
        Energie non calorifique échangé au cours d'une transformation :
            $$ \boxed{\delta W = - P_{ext}\ \re d V} $$

    \subsection*{Entropie : $\bm{S}$}
        L'entropie est une fonction d'état extensive mesurée en joule par Kelvin.\\
        C'est est une grandeur physique qui caractérise le \textbf{degré de désorganisation d'un système}.
        $$ \boxed{\diff S_\sigma = \frac{\delta Q_{\sigma,\text{rév}}}{T_\sigma}} $$
    
    \subsection*{Enthalpie $\bm{H}$}
        L'enthalpie est une fonction d'état extensive mesurée en joule.
            $$ \boxed{H = U + P V} $$
            $$ \boxed{ \diff H = C_p \, \diff T + (h + V) \, \diff P } $$

    \subsection*{Enthalpie Libre $\bm{G}$ (Gibbs)}
        L'enthalpie libre $G$ est une fonction d'état extensive. $\Delta G$ mesure le travail maximal pouvant être
        extrait d'un système à température et pression constantes.
            $$ \boxed{ G = H - TS } $$
            $$ \boxed{ \diff G = - S \, \diff T + V \, \diff P } $$

    \subsection*{Le potentiel chimique $\bm{\mu}$}
        Le potentiel chimique est une grandeur intensive définie comme l'évolution de l'énergie
        libre du système lors de l'ajout d'une mole de matière, à température et pression constantes.
            $$ \boxed{ \mu_i = \left(\frac{\partial G}{\partial n_i}\right)_{T,P,n_{j(j \ne i)}} } $$
            $$ \boxed{ \mu_i(T,P) = \mu_i^{\circ}(T) + RT\ln{(\alpha_i)} }$$

    \end{multicols}

\clearpage

\section{Capacité Calorifique d'un système}

    La capacité thermique est une grandeur extensive en $J.K^{-1}$.\\
    \textit{C'est l'énergie à apporter pour élever la température d'un degré.}
    \vspace{3mm}

    \textbf{Pour les gaz}, comme ils sont compressibles, on distingue deux modes de chauffage :
    \vspace{3pt}
    \begin{itemize}[label=\textbullet]
        \item \textbf{Chauffage isobare :} on définis $\bm{C_p}$ (capacité calorifique du gaz à pression constante).
        \item \textbf{Chauffage isochore :} on définis $\bm{C_v}$ (capacité calorifique du gaz à volume constant).
    \end{itemize}

    \subsection*{Coefficient de Laplace d'un gaz : $\bm{\gamma}$}

    \textit{Le coeficient de Laplace (ou l'indice adiabatique) d'un gaz est défini comme le rapport de
    ses capacités thermiques à pression constante et à volume constant :} $\boxed{\gamma = \dfrac{C_p}{C_v}}$

    Avec la \textbf{relation de Mayer} $C_P - C_V = R$ \ on peut en déduire que pour les gaz monoatomiques, $\gamma \approx 1,67$ et pour les gaz diatomiques, $\gamma \approx 1,4$.




\section{Les Principes de la Thermodynamique}

    \textit{Un principe est une loi générale qui s'applique à tous les systèmes thermodynamiques. Il n'a pas été démontré, mais aucun résultat experimental ne l'a contredit.}

    \begin{multicols}{2}

        \subsection*{Le Premier Principe}
        Conservation de l'énergie : $\boxed{\diff U = \delta Q + \delta W}$

        \subsection*{Le Second Principe}
        L'entropie d'un système isolé ne peut qu'augmenter : $\boxed{\diff S \geq 0}$ (créable et indestructible).

        \subsection*{Le Troisième Principe}
        L'entropie d'un cristal parfait à 0 K est nulle : $\boxed{S(0) = 0}$\\
        L'entropie d'un système ne peut pas être inférieure à 0 : $\boxed{S \geq 0}$

    \end{multicols}



\section{Gaz parfaits}

    \begin{multicols}{2}
        
        \subsection*{Loi des Gazs Parfaits}
            $$ \boxed{PV = nRT} $$
            Avec $P$ en Pascals, $V$ en $m^3$, $n$ en mol, $ R=\qty{8.314}{J mol^{-1} K^{-1}} $ et $T$ en Kelvin. 
        
        \subsection*{Energie interne}
            $$ \boxed{ \diff U_{GP} = C_v \, \diff T } $$
            Avec $\Delta U$ en Joules.

        \subsection*{Chaleur $\bm{Q}$}
            Pour un gaz parfait, comme la chaleur latente d'expension $l=P$ et que la chaleur latente de compression $h=-V$ on a :
                $$ \boxed{ Q_{\text{GP,rév}} = C_v \, \diff T + P \, \diff V } $$
                $$ \boxed{ Q_{\text{GP,rév}} = C_p \, \diff T - V \, \diff P } $$

        \subsection*{Enthalpie}
            $$ \boxed{ \diff H_{GP} = C_p \, \diff P } $$

    \end{multicols}

\clearpage

\section{Transformation d'un gaz parfait entre deux états A et B}

    Lors d'une transformation, le système peut échanger de l'énergie avec le milieu extérieur
    sous forme de travail (noté $W$) ou de chaleur (noté $Q$).\\
    On distingue deux types de transformations:
    \begin{itemize}
        \item \textbf{réversible :} cas hypothétique d'une transformation infiniment lente. Donc le système est constament à l'équilibre avec l'extérieur.
        \item \textbf{irréversible :} Transformation "réelle" hors équilibre avec l'extérieur.
    \end{itemize}


    \subsection{Transformation Adiabatique}

        \textbf{Pas d'échange de chaleur avec l'extérieur:} $\boxed{Q = 0}$\\ 
        $ \Longrightarrow \quad \Delta U = W = n\Cv\Delta T $ \quad le travail se comporte comme une fonction d'état ($W_{\text{rév}} = W_{\text{irrév}}$).

        \subsubsection{Réversible}

            \begin{itemize}[label=\textbullet]
                \item \textbf{Relation de Poisson :} $PV^\gamma = \re{cste}$
                \item \textbf{Relation de Laplace :} $T V^{\gamma - 1} = \re{cste}$
                \item \textbf{Relation de Clapeyron :} $T^\gamma P^{1 - \gamma} = \re{cste}$
            \end{itemize}

    \subsection{Transformation Isotherme}

        Transformation à température constante ($T_A=T_B$).
        
        Comme $ \Delta U_{GP} = 0 \quad (\Delta T = 0) $ on a $W_{AB} = -Q_{AB}$.

        \subsubsection{Réversible}
            $$ W_{AB} = \int_{V_A}^{V_B} -P_{ext} \, \diff V = \int_{V_A}^{V_B} -P_\sigma \, \diff V = \int_{V_A}^{V_B} -\frac{nRT}{V} \, \diff V = -nRT \int_{V_A}^{V_B} \frac{\diff V}{V} = -nRT \ln{\left(\frac{V_B}{V_A}\right)} $$
            
            $$ \Delta S_\sigma = \int_A^B \frac{\delta Q_{\sigma,\text{rév}}}{T_\sigma} = \int_{V_A}^{V_B} \frac{P_\sigma \, \diff V}{T_\sigma} = \int_A^B \frac{-\delta W_{\text{rév}}}{T_\sigma} = n R \ln{\left(\frac{V_B}{V_A}\right)} $$

            $$ \Delta S_{\text{ext}} = \int_A^B \frac{\delta Q_{\text{ext,rév}}}{T_{\text{ext}}} = \int_A^B \frac{-\delta Q_{\sigma,\text{rév}}}{T_{\text{ext}}} = - n R \ln{\left(\frac{V_B}{V_A}\right)} $$

            On retrouve bien que pour une transformation réversible: $\Delta U_u = 0$. (On aurait pu directement dire que $\Delta U_{\text{ext}} = -\Delta U_\sigma$)

        \subsubsection{Irréversible}
            $$ W_{AB'} = \int_{V_A}^{V_B} -P_{ext} \, \diff V = -P_{ext} \int_{V_A}^{V_B} \diff V = -P_{ext} (V_B - V_A) = P_{ext} (V_A- V_B) $$

            $$ \Delta S_\sigma' = \int_A^B \frac{\delta Q_{\sigma,\text{rév}}}{T_\sigma} = \int_{V_A}^{V_B} \frac{P \, \diff V}{T_\sigma} = \int_A^B \frac{-\delta W_{AB}}{T_\sigma} = n R \ln{\left(\frac{V_B}{V_A}\right)} $$

            On retrouve bien que $\Delta S_\sigma' = \Delta S_\sigma$ puisque l'entropie est une fonction d'état.
            
            $$ \Delta S_{\text{ext}}' = \int_A^B \frac{\delta Q_{\text{ext,rév}}}{T_{\text{ext}}} = \int_A^B \frac{-\delta Q_{\sigma,\text{irrév}}}{T_{\text{ext}}} = \int_A^B \frac{\delta W_{\sigma,\text{irrév}}}{T_{\text{ext}}} = -\frac{P_B (V_B - V_A)}{T_{\text{ext}}} $$

            $$ \Delta U_u' = \frac{-W_{\text{rév}}}{T_{\text{ext}}} + \frac{W_{\text{irrév}}}{T_{\text{ext}}} = \frac{W_{\text{irrév}} - W_{\text{rév}}}{T_{\text{ext}}} > 0 $$

    \subsection{Transformation Isobare}
        
        Transformation à pression constante ($P_A=P_B$).

        \subsubsection{Réversible}
            $$ Q_{AB} = n \Cp \, \Delta T - V \, \Delta P = n \Cp \, \Delta T $$
        
        \subsubsection{Irréversible}
            $$ W_{AB'} = \int_{V_A}^{V_B} -P_{\text{ext}} \diff V = -P_{\text{ext}}(V_B-V_A) = P_{\text{ext}}(V_A-V_B) $$

    \subsection{Transformation Isochore}

        Transformation à volume constant ($V_A=V_B$).

        Dans tout les cas : $ W_{AB} = 0 $ car $ \diff V = 0 $. Donc on a $ \Delta U_{GP} = Q_{AB} $.
        
        Dans tout les cas : $Q_{AB} = \Delta H_{GP} = C_p \, \Delta T $
                
        \subsubsection{Réversible}

             $$ Q_{AB} = n \Cp \, \Delta T - V \, \Delta P = n \Cp \, \Delta T $$

        \clearpage

    \section{Mélange binaires}
        
        \subsection{Loi de Dalton}
            
            \textbf{La loi de Dalton peut être appliqué à des mélanges de gaz parfaits} uniquement.
                $$ P_{tot} = P_A + P_B + \cdots = \sum_i P_i $$
                $$ \boxed{ P_i = y_i P_{i}^* } $$

        \subsection{Loi de Raoult}
            
            \textbf{La loi de Raoult peut être appliqué à des solutions idéales} uniquement.
                $$ \boxed{ P_A = x_A P_A^* } $$

            On l'utilise lors d'un mélange binaire dit "idéale" pour établir le diagramme binaire isotherme ou isobare du mélange idéal.
            \textit{Il y a plus souvent un écart positif que négatif à cette loi.}
            
            Ecart positif si à $P$ donné, $T_{\text{eb, réel}} < T_{\text{eb, idéal (Raoult)}}$ .
        

        \subsection{Loi de Henry}
            \textbf{La loi de Henry est utilisé pour déterminer la fraction molaire d'un composé fortement diluée dans un solvant B.} Par exemple un gaz dans de l'eau (solubilité toujours faible).
            Le solvant B obéira alors à la loi de Raoult.
                $$ \boxed{ P_A = x_A K_{H_A} }$$
        
        \subsection{Règles des moments chimiques}
            
            \begin{table}[!ht]
                \centering
                \renewcommand{\arraystretch}{2.5}
                \begin{tabular}{|c|c|}
                \hline
                \textbf{Nombre de moles} & \textbf{Masse} \\
                \hline

                \textbf{Composition en fraction molaire} 
                &
                \textbf{Composition en fraction massique} 
                \\
                \hline
                
                % --- Axe gauche : molaire ---
                \begin{tikzpicture}[x=6cm, y=1cm, >=latex]
                    \draw[->] (0,0) -- (1.1,0);
                    \foreach \x/\label in {0/x_B, 0.5/x_M, 1/y_B}
                    {
                    \draw (\x,0.05) -- (\x,-0.05) node[below] {$\label$};
                    }
                    \node[above] at (0,0) {$L$};
                    \node[above] at (0.5,0) {$M$};
                    \node[above] at (1,0) {$V$};
                \end{tikzpicture}
                &
                % --- Axe droit : massique ---
                \begin{tikzpicture}[x=6cm, y=1cm, >=latex]
                    \draw[->] (0,0) -- (1.1,0);
                    \foreach \x/\label in {0/w_{B,L}, 0.5/w_M, 1/w_{B,V}}
                    {
                    \draw (\x,0.05) -- (\x,-0.05) node[below] {$\label$};
                    }
                    \node[above] at (0,0) {$L$};
                    \node[above] at (0.5,0) {$M$};
                    \node[above] at (1,0) {$V$};
                \end{tikzpicture}
                \\
                \hline

                % -- FORMULES MOLAIRES --
                \(
                \displaystyle
                \frac{n_L}{n_V}
                = \frac{y_B - x_M}{x_M - x_B}
                = \frac{\overline{MV}}{\overline{LM}}
                \)
                &
                \(
                \displaystyle
                \frac{m_L}{m_V}
                = \frac{w_{B,V} - w_M}{w_M - w_{B,L}}
                = \frac{\overline{MV}}{\overline{LM}}
                \)

                \\[2mm]
                \(
                \displaystyle
                \frac{n_L}{n_{\text{tot}}}
                = \frac{y_B - x_M}{y_B - x_B}
                = \frac{\overline{MV}}{\overline{LV}}
                \)
                &
                \(
                \displaystyle
                \frac{m_L}{m_{\text{tot}}}
                = \frac{w_{B,V} - w_M}{w_{B,V} - w_{B,L}}
                = \frac{\overline{MV}}{\overline{LV}}
                \)

                \\[2mm]

                \(
                \displaystyle
                \frac{n_V}{n_{\text{tot}}}
                = \frac{x_M - x_B}{y_B - x_B}
                = \frac{\overline{LM}}{\overline{LV}}
                \)
                &
                \(
                \displaystyle
                \frac{m_V}{m_{\text{tot}}}
                = \frac{w_M - w_{B,L}}{w_{B,V} - w_{B,L}}
                = \frac{\overline{LM}}{\overline{LV}}
                \)
                \\[2mm]
                \hline

                \end{tabular}
                \caption{Les différentes formulations de la règle des moments chimiques}

            \end{table}

            Pour établir ces relations, on est parti de la relation suivante:
                $$ m_B = \bm{w_M \, m_{\text{tot}}} = m_{B,L} + m_{B,V} = w_{B,L} \, m_L + w_{B,V} \, m_V = \bm{ w_{B,L} \, m_L + w_{B,V} \, (m_{\text{tot}} - m_L) } $$
                $$ \Longleftrightarrow \quad m_{\text{tot}} (w_M - w_{B,V}) = m_L \, (w_{B,L} - w_{B,V}) \quad \Longrightarrow \quad \bm{ m_L = m_{\text{tot}} \frac{w_M - w_{B,V}}{w_{B,L} - w_{B,V}} }$$

            Ou encore en molaire :
                $$ n_B = \bm{ x_M \, n_{\text{tot}} } = n_{B,L} + n_{B,V} = x_B \, n_L + y_B \, n_V = \bm{ x_B \, n_L + y_B \, (n_{\text{tot}} - n_L) } $$ 
                $$ \Longleftrightarrow \quad n_{\text{tot}} (x_M - y_B) = n_L (x_B - y_B) \quad \Longrightarrow \quad \bm{ n_L = n_{\text{tot}} \frac{x_M - y_B}{x_B - y_B} }$$
        \begin{tikzpicture}[x=10cm, y=1cm, >=latex]

            % Axe des compositions
            \draw[->] (0,0) -- (1.05,0) node[below] {$x$};

            % Axe des températures
            \draw[->] (0,0) -- (0,4.5) node[left] {$T$};
            
            % Positions des 3 points
            \def\xV{0.25}
            \def\xM{0.50}
            \def\xL{0.72}

            % Courbe de rosée (haute)
            \draw[thick]
            plot[smooth] coordinates {
            (0,1) (\xV, 2) (1,4)
            } node[left=20pt] {Courbe de rosée};

            % Courbe de vaporisation (basse)
            \draw[thick]
            plot[smooth] coordinates {
            (0,1) (\xL, 2) (1,4)
            } node[below=50pt] {Courbe de vaporisation};

            % Graduation 0 à 1
            \foreach \x/\label in {0/0 , 0.2/0.2 , 0.4/0.4 , 0.6/0.6 , 0.8/0.8 , 1/1}
            \draw (\x,0.05) -- (\x,-0.05) node[below=2pt] {\label};
            
            % Graduation 0 à 100°C
            \foreach \y/\label in {0/0 , 1/60 , 2/80 , 3/100 , 4/120}
            \draw (-0.005,\y) -- (0.005,\y) node[left=12pt] {\label};
            
            
            % Points
            \fill (\xL,2) circle (2pt) node[below] {$L$};
            \fill (\xM,2) circle (2pt) node[above] {$M$};
            \fill (\xV,2) circle (2pt) node[above] {$V$};

            % Projections verticales
            \draw[dashed, red] (\xL,2) -- (\xL,0) node[below=2pt, blue] {$x_B$};
            \draw[dashed, red] (\xM,2) -- (\xM,0) node[below=2pt, green!60!black] {$x_M$};
            \draw[dashed, red] (\xV,2) -- (\xV,0) node[below=2pt, purple] {$y_B$};

            % Segment entre L et V
            \draw[thick] (\xL,2) -- (\xV,2);

        \end{tikzpicture}

      

        \clearpage

    \section{Réactions chimiques}

        \subsection{Définitions}
            On considère la réaction chimique suivante:
                $$ \nu_{A_1} A_1 + \nu_{A_2} A_2 + \cdots + \nu_{A_i} A_i \, \rightleftarrows \, \nu_{B_1} B_1 + \nu_{B_2} B_2 + \cdots + \nu_{B_j} B_j $$


            \subsubsection*{Rendement $r$ ou $\eta$}

                $$ \boxed{ r = \frac{n_{j \text{ obtenues à l'équilibre}}}{n_{j \text{ obtenues si réaction totale}}} } $$

            \subsubsection*{Taux d'avancement $\tau$}

                $$ \boxed{ \tau = \frac{\xi_{\text{équilibre}}}{\xi_{\text{max si réaction totale}}} } $$

            \subsubsection*{Quotient de réaction $Q$}

                $$ \boxed{ Q = \dfrac{\prod_{j=1}^{n_{\text{produit}}} a_{B_j}^{\nu_{B_j}}}{\prod_{i=1}^{n_{\text{réactif}}} a_{A_i}^{\nu_{A_i}}} } $$ 

            \subsubsection*{Constante d'équilibre $K_T^{\circ}$}

                $$ \boxed{ K_T^{\circ} = \dfrac{\prod_{j=1}^{n_{\text{produit}}} (a_{B_j})_{\text{éq}}^{\nu_{B_j}} }{\prod_{i=1}^{n_{\text{réactif}}} (a_{A_i})_{\text{éq}}^{\nu_{A_i}}} = e^{\frac{-\Delta G_T^{\circ}}{RT}} }$$

            \subsubsection*{Enthalpie Libre $\Delta G_{T,P}$}
                L'enthalpie libre standard notée $\Delta_r G_T^{\circ}$ peut-être calculé comme ceci :
                    $$ \boxed{ \Delta_r G_T^{\circ} = \Delta_r H_T^{\circ} - T \Delta_r S_T^{\circ} = -RT \ln{(K_T^{\circ})} } $$

                L'expression générale pour $\Delta_r G_{T,P}$ est :
                    $$ \boxed{ \Delta_r G_{T,P} = \Delta_r G_T^{\circ} + RT \ln{(Q)} = -RT \ln{(K_T^{\circ})} + RT \ln{(Q)} = RT \ln{\left(\frac{Q}{K_T^{\circ}}\right)} } $$

                    $$ \boxed{ \Delta_r G_{T,P} = -n F (E_2-E_1) } $$

                On peut retenir que le sens de la réaction est déterminé par :
                
                \begin{itemize}[label=\textbullet]
                    \item $ \bm{ \Delta_r G_{T,P} < 0 \Longleftrightarrow Q < K_T^{\circ} } $: Le système évoluera dans le sens 1, jusqu'à ce que $ Q = K_T^{\circ} $.
                    \item $ \bm{ \Delta_r G_{T,P} > 0 \Longleftrightarrow Q > K_T^{\circ} } $: Le système évoluera dans le sens 2, jusqu'à ce que $ Q = K_T^{\circ} $.
                    \item $ \bm{ \Delta_r G_{T,P} = 0 \Longleftrightarrow Q = K_T^{\circ} } $: Le système est à l'équilibre.
                \end{itemize}

            \subsubsection*{Potentiel d'un electrode $E$}
                Les potentiels standards sont tabulés et sont notés $E^\circ$.
                Grâce à la \textbf{relation de Nernst}, on peut calculer les potentiels des différents électrode:
                    $$ E_{\text{Ox/Red}} = E^\circ_{\text{Ox/Red}} + \frac{RT}{nF} \times \ln{\left( \frac{\prod_i (a_{A_i})^{v_i}}{\prod_j (a_{B_j})^{v_j}}\right)} $$
                
                \textbf{A l'équilibre, il y a égalité entre les deux potentiels des deux électrodes.}
\end{document}